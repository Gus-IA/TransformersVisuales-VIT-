...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, ix):
        # ORIGINAL: return vector de 784 elementos
        # return torch.tensor(self.X.iloc[ix].values).float()

        # ✔ NUEVO: devolver imagen como (1, 28, 28)
        img = torch.tensor(self.X.iloc[ix].values).float().view(1, 28, 28)
        label = torch.tensor(self.y.iloc[ix]).long()
        return img, label
...
imgs = imgs.view(-1, 1, 28, 28)  # (64, 1, 28, 28)
